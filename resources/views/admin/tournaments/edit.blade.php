            ...
            <form action="{{ route('tournaments.update', $tournament) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="date" name="start_date" value="{{ old('start_date', $tournament->start_date?->format('Y-m-d')) }}" class="w-full px-4 py-3 rounded-xl border {{ $errors->has('start_date') ? 'border-red-500' : 'border-gray-200' }} focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none">
                    </div>
                    <div>
                        <label class="block text-gray-700 font-bold mb-2">End Date</label>
                        <input type="date" name="end_date" value="{{ old('end_date', $tournament->end_date?->format('Y-m-d')) }}" class="w-full px-4 py-3 rounded-xl border {{ $errors->has('end_date') ? 'border-red-500' : 'border-gray-200' }} focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none">
                    </div>
                </div>
