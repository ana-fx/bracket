...
@else
    <!-- Bracket Container -->
    <div class="relative min-h-[500px] flex justify-center py-12 overflow-x-auto" id="bracket-container">

        {{-- SVG Layer for Connectivity --}}
        <svg class="absolute inset-0 w-full h-full pointer-events-none z-0" id="connector-svg" style="min-width: 100%; min-height: 100%;">
            <!-- Lines will be injected here via JS -->
        </svg>

        {{-- Flex container for Rounds --}}
        <div class="flex gap-16 px-8 relative z-10">
                    ...
                    @foreach($matches as $match)
                        ...
                        <div class="relative flex flex-col justify-center gap-6 group match-node"
                             ...
                             data-winner-id="{{ $match->winner_id }}">
                                ...
                                {{-- P1 Card --}}
                                <div id="match-{{ $match->id }}-p1" class="relative bg-white rounded-xl shadow-sm ring-1 ring-gray-100 p-3 flex items-center justify-between {{ isset($isAdmin) && $isAdmin ? 'cursor-pointer hover:ring-primary/50 hover:shadow-md transition-all' : '' }}"
                                     @if(isset($isAdmin) && $isAdmin) @click='openModal(@json($match))' @endif>

                                    <div class="flex items-center gap-3 overflow-hidden">
                                        <div class="w-8 h-8 rounded-full bg-gray-50 flex items-center justify-center text-xs text-gray-400 font-bold border border-gray-100 flex-shrink-0">
                                        ...
                                        <div class="flex flex-col min-w-0">
                                            ...
                                            @if($match->participant1)
                                                <span class="text-[10px] text-gray-400 font-medium truncate">{{ $match->participant1->affiliation }}</span>
                                            @endif
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex items-center gap-2 pl-2">
                                         @if($match->winner_id && $match->winner_id == $match->participant_1_id)
                                            <svg class="w-4 h-4 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" /></svg>
                                        @endif
                                        @if($match->participant_1_score > 0 || $match->participant_2_score > 0)
                                            <span class="font-bold text-lg {{ $match->winner_id == $match->participant_1_id ? 'text-green-600' : 'text-gray-300' }}">
                                                {{ $match->participant_1_score }}
                                            </span>
                                        @endif
                                    </div>

                                    {{-- Horizontal link removed --}}
                                </div>

                                {{-- P2 Card --}}
                                <div id="match-{{ $match->id }}-p2" class="relative bg-white rounded-xl shadow-sm ring-1 ring-gray-100 p-3 flex items-center justify-between {{ isset($isAdmin) && $isAdmin ? 'cursor-pointer hover:ring-primary/50 hover:shadow-md transition-all' : '' }}"
                                     @if(isset($isAdmin) && $isAdmin) @click='openModal(@json($match))' @endif>
                                     ...
                                     <div class="flex items-center gap-3 overflow-hidden">
                                        ...
                                        <div class="flex flex-col min-w-0">
                                            ...
                                            @if($match->participant2)
                                                <span class="text-[10px] text-gray-400 font-medium truncate">{{ $match->participant2->affiliation }}</span>
                                            @endif
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex items-center gap-2 pl-2">
                                        ...
                                        @endif
                                        @if($match->participant_1_score > 0 || $match->participant_2_score > 0)
                                            <span class="font-bold text-lg {{ $match->winner_id == $match->participant_2_id ? 'text-green-600' : 'text-gray-300' }}">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
             ...
             // Also redraw after a short delay to allow for fonts/layout to settle
        ...
        function drawConnectors() {
            ...
            matches.forEach(match => {
                ...
                // Calculate Points relative to Container Scroll
                const scrollX = container.scrollLeft;
                const scrollY = container.scrollTop;

                const p1RightX = (p1Rect.right - containerRect.left) + scrollX;
                const p1Y = (p1Rect.top + p1Rect.height / 2 - containerRect.top) + scrollY;

                const p2RightX = (p2Rect.right - containerRect.left) + scrollX;
                const p2Y = (p2Rect.top + p2Rect.height / 2 - containerRect.top) + scrollY;

                const nextLeftX = (nextRect.left - containerRect.left) + scrollX;
                const nextY = (nextRect.top + nextRect.height / 2 - containerRect.top) + scrollY;

                // Fork Logic
                // Midpoint between current match end and next match start
                // We want the vertical bar to be somewhere in between.
                const forkX = p1RightX + 20; // Move out 20px from cards
                ...
                const highlightColor = '#A78BFA'; // Primary Color
                const width = '2';

                // Path 1: P1 -> Fork
                // M p1RightX p1Y L forkX p1Y
                const d1 = `M ${p1RightX} ${p1Y} L ${forkX} ${p1Y}`;
                createPath(svg, d1, isP1Winner ? highlightColor : defaultColor, width);

                // Path 2: P2 -> Fork
                // M p2RightX p2Y L forkX p2Y
                const d2 = `M ${p2RightX} ${p2Y} L ${forkX} ${p2Y}`;
                createPath(svg, d2, isP2Winner ? highlightColor : defaultColor, width);

                // Path 3: Vertical Bar (Connects P1 Y and P2 Y at ForkX)
                // From Top Y to Bottom Y
                const topY = Math.min(p1Y, p2Y);
                const bottomY = Math.max(p1Y, p2Y);

                // Vertical Segment Top (P1 side) -> Center
                const midY = (p1Y + p2Y) / 2;

                // If P1 wins, highlight top half of vertical bar
                const d3Top = `M ${forkX} ${p1Y} L ${forkX} ${midY}`;
                createPath(svg, d3Top, isP1Winner ? highlightColor : defaultColor, width);

                // If P2 wins, highlight bottom half of vertical bar
                const d3Bottom = `M ${forkX} ${p2Y} L ${forkX} ${midY}`;
                createPath(svg, d3Bottom, isP2Winner ? highlightColor : defaultColor, width);


                // Path 4: Center -> Next Match
                // M forkX midY L nextLeftX nextY
                // Orthogonal: forkX midY -> midX midY -> midX nextY -> nextX nextY
                // Or just: forkX midY -> nextLeftX-20 midY -> nextLeftX-20 nextY -> nextLeftX nextY

                const stepX = nextLeftX - 20;
                // If the next match is horizontally far, usually we just go straight if Y aligns, or step.
                // Current bracket logic: Next match is usually centered Y-wise related to its children.
                // So midY ~ nextY usually.
